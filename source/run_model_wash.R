...
## must run generate_wash_rasters.R second
...
library(foreach)
library(doParallel)
cl <- makeCluster(3, outfile="target_model_wash_log.txt")
...
targets_dir <- paste0("country_targets/", ifelse(sensitivityMod %in% c("baseline", "high_campaignFreq", "low_campaignFreq", "high_coverage", "low_coverage"), sensitivityMod, "baseline"), "/")
dir.create(targets_dir, showWarnings = FALSE)

###############################
## Set inputs for model runs 
alloc_strategy_ls <- c("optimumWat", "optimumWash", "optimumSan") 
